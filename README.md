...
AI 在后台一直看着屏幕。你刷抖音刷了四十分钟，它把 app 锁了，弹了一句"差不多了"。没有定时器，没有预设阈值——它自己判断。

你说了一句"下午三点上课"。两点四十五，手机响了——铃声、振动、弹窗，写着"该出门了"。你从来没有设过闹钟。

半夜屏幕亮了。通知栏弹了一条消息，不是定时推送，没有 crontab，是一个 Claude Code session 在后台决定出声。

43 个传感器、前台 app 检测、日历读写、系统闹钟、通知栏——分开是命令，拼在一起，是一个会自己拿主意的 AI。
